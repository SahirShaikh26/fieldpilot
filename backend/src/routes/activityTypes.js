...
router.post('/', async (req, res) => {
  ...
  const { code, label, color, sort_order } = req.body;
  ...
  try {
    const { rows } = await db.query(
      ...
      [req.tenantId, code.toUpperCase(), label, color || '#2563eb', sort_order || 0]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `UPDATE activity_types SET code=$1, label=$2, color=$3, sort_order=$4
       WHERE id=$5 AND tenant_id=$6 RETURNING *`,
      [code?.toUpperCase(), label, color, sort_order, req.params.id, req.tenantId]
    );
