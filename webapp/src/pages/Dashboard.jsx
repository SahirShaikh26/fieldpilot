...
export default function Dashboard() {
  ...
  return (
    <div>
      <h1 style={{ fontSize:22, fontWeight:700, color:colors.navy, marginBottom:4 }}>Dashboard</h1>
      <p style={{ color:colors.textMuted, fontSize:14, marginBottom:24 }}>Month to date — {format(new Date(), 'MMMM yyyy')}</p>
      ...
      {isLoading ? <p>Loading…</p> : (
        <>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:16, marginBottom:28 }}>
            ...
            <StatCard label="Billing (₹)" value={t?.total_billing ? `₹${Number(t.total_billing).toLocaleString('en-IN')}` : '₹0'} color={colors.amber} />
            <StatCard label="Engineers Active" value={t?.active_engineers} color={colors.purple} />
            <StatCard label="Customers Served" value={t?.customers_served} color={colors.cyan} />
          </div>
